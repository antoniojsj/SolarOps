...
// Import the new semantic renamer function
import { renameLayersWithHtmlSemantics } from "./semanticRenamer";
  ...
  if (msg.type === "rename-layers-with-html-semantics") {
    try {
      ...
      figma.ui.postMessage({
        ...
        payload: { success: true, renamedCount }
      });
    } catch (error) {
      console.error("[Controller] Error during semantic rename:", error);
      figma.ui.postMessage({
        type: "rename-complete",
